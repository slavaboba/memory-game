...
import (
	...
	"memory-master/internal"
	"memory-master/shared"
)

var game shared.Help
var myApp fyne.App

func ShowEntryWindow() {
	myApp = app.New()
	window := myApp.NewWindow("Memory Master Entry")
	window.SetContent(createEntryMenu(window))
	window.Resize(fyne.NewSize(800, 600))
	window.Show()
	myApp.Run()
}

func createEntryMenu(window fyne.Window) fyne.CanvasObject {
	title := widget.NewLabelWithStyle("Memory Master Entry", fyne.TextAlignCenter, fyne.TextStyle{Bold: true})

	entryname := widget.NewEntry()
	entryname.SetPlaceHolder("Enter your name:")
	entryIP := widget.NewEntry()
	entryIP.SetPlaceHolder("Enter IP of your server:")

	EntryButton := widget.NewButton("Войти", func() {
		game.UserName = entryname.Text
		if game.UserName == "" {
			game.UserName = "unnamed"
		}
		if game.UserName == "Oleg" {
			game.UserName = "silly guy"
		}
		game.IP = fmt.Sprintf("%v:8080", entryIP.Text)
...
func showScores(window fyne.Window) {
	Players, _, _ := internal.Writing(game.IP)

	for i := 0; i < len(Players); i++ {
		game.PlNames[i] = Players[i].Name
		game.PlScores[i], _ = strconv.Atoi(Players[i].Text)
	}
	scores := []shared.Score{
		{PlayerName: game.PlNames[0], Score: game.PlScores[0]},
		{PlayerName: game.PlNames[1], Score: game.PlScores[1]},
		{PlayerName: game.PlNames[2], Score: game.PlScores[2]},
		{PlayerName: game.PlNames[3], Score: game.PlScores[3]},
		{PlayerName: game.PlNames[4], Score: game.PlScores[4]},
		{PlayerName: game.PlNames[5], Score: game.PlScores[5]},
		{PlayerName: game.PlNames[6], Score: game.PlScores[6]},
		{PlayerName: game.PlNames[7], Score: game.PlScores[7]},
		{PlayerName: game.PlNames[8], Score: game.PlScores[8]},
		{PlayerName: game.PlNames[9], Score: game.PlScores[9]},
	}
...
func showAchievements(window fyne.Window) {
	...
	achievements := []shared.Achievement{
		{Name: "Новичок", Description: "Завершить лёгкий уровень", Unlocked: game.Аchives[0]},
		{Name: "Знаток", Description: "Пройти средний уровень за 3 минуты", Unlocked: game.Аchives[1]},
		{Name: "Мастер памяти", Description: "Пройти сложный уровень без ошибок.", Unlocked: game.Аchives[2]},
		{Name: "Легенда", Description: "Победить на экспертном уровне.", Unlocked: game.Аchives[3]},
	}
	...
	list := widget.NewList(
		...
		func(i widget.ListItemID, o fyne.CanvasObject) {
			...
			if achievements[i].Unlocked {
				title.SetText(achievements[i].Name + "Выполнено ✓")
			}
...
func exit() {
	if game.MaxScore != 0 {
		internal.Writer(game.UserName, fmt.Sprint(game.MaxScore), game.IP)
	}
	myApp.Quit()
}
